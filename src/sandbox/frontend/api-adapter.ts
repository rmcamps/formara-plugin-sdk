...
function getBaseUrl(): string {
  ...
  if (standalone) {
    ...
    if (typeof window !== 'undefined' && (window as any).__PLUGIN_BACKEND_URL__) {
      return (window as any).__PLUGIN_BACKEND_URL__;
    }
    
    // Fallback: usar puerto si está configurado
    const backendPort = (typeof window !== 'undefined' && (window as any).__PLUGIN_BACKEND_PORT__) || '4001';
...
export function api(path: string): string {
  ...
  if (!standalone && typeof window !== 'undefined') {
    ...
    const coreApi = (window as any).__FORMARA_API__;
    ...
    const envBase = (import.meta.env.VITE_API_URL as string) || window.location.origin;
